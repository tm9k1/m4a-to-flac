...
from music_flac.layout import flac_mirror_path
...
def cmd_plan(args: argparse.Namespace, cfg: AppConfig) -> int:
    ...
    for t, dest in pairs:
        print(f"{t.relative_path}")
        print(f"  -> {flac_mirror_path(t.relative_path)}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="music-flac",
        description="Scan a music library and mirror the same tree under a FLAC folder (via API).",
    )
